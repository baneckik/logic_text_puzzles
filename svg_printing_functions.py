...
def draw_into_rectangle(dwg, X, Y, text, rec_h, rec_w, angle=0):
    width = rec_h
...
def draw_grid(puzzle1, dwg, X=30, Y=30, puzzle_h=400, fname="undefined.svg"):
    ...
    for row in range(N_rows):
        ...
        dwg.add(dwg.rect((X + text_box_size + row * box_size, 870 - Y - (K_cat + 0.5) * box_size),
                         (box_size, text_box_size + (N_rows - row) * box_size),
                         stroke='black', stroke_width=2, fill='none')
                )
    for row in range(N_rows):
        for k in range(k_cat):
            dwg.add(dwg.rect((X, 870 - Y - (row + 1) * box_size),
                             (text_box_size + (row + 1) * box_size, k * box_size / k_cat),
                             stroke='black', stroke_width=1, fill='none')
                    )
            dwg.add(dwg.rect((X + text_box_size + row * box_size, 870 - Y - (K_cat + 0.5) * box_size),
                             (k * box_size / k_cat, text_box_size + (N_rows - row) * box_size),
                             stroke='black', stroke_width=1, fill='none')
                    )
    ...
    for i in range(len(categories)):
        ...
        for col, name in enumerate(nazwy):
            ...
            if 'groups' in categories[i] and len(unique(categories[i]["groups"])) > 1:
                ...
                widths.append(textwidth(name2, width2))
            else:
    ...
    for i, category in enumerate(categories):
        ...
        if category['cross_bar'] != "":
            ...
            if i != 0:
                dwg.add(
                    dwg.rect((X + text_box_size + box_size * (i - 1), 870 - Y - (K_cat - 1) * box_size - text_box_size),
                             (box_size, box_size / k_cat),
                             stroke='black', stroke_width=2, fill='white')
                    )
            ...
            if i != 0:
                draw_into_rectangle(dwg, X + text_box_size + box_size * (i - 1) + odstep * width, 870 - Y - (
                            K_cat - 1) * box_size - text_box_size + box_size / k_cat - odstep * width, text.upper(),
                                    width, box_size)
            # cross bar text on the left
            if i != 1:
                if i != 0:
                    draw_into_rectangle(dwg, X + box_size / k_cat - odstep * width,
                                        870 - Y - (i - 2) * box_size - odstep * width, text.upper(), width, box_size,
                                        angle=270)
                else:
                    draw_into_rectangle(dwg, X + box_size / k_cat - odstep * width,
                                        870 - Y - (K_cat - 2) * box_size - odstep * width, text.upper(), width,
                                        box_size, angle=270)
        ...
        for i, name in enumerate(nazwy):
            ...
            # horizontal texts:
            if miejsce == 1:
                draw_into_rectangle(dwg, X + odstep * width + odstep2,
                                    870 - Y - (K_cat - 1) * box_size + (i + 1) * width - odstep * width, name.upper(),
                                    width, space_size)
            elif miejsce != 2:
                draw_into_rectangle(dwg, X + odstep * width + odstep2,
                                    870 - Y - (miejsce - 2) * box_size + (i + 1) * width - odstep * width, name.upper(),
                                    width, space_size)
            # vertical texts:
            if miejsce != 1:
                draw_into_rectangle(dwg,
                                    X + text_box_size + (i + 1) * width - odstep * width + (miejsce - 2) * box_size,
                                    870 - Y - (K_cat - 1) * box_size - odstep * width, name.upper(), width, space_size,
                                    angle=270)
...
def draw_on_canvas(puzzle1, dwg, fname="undefined.svg", X=30, Y=30, puzzle_h=400):
    ...
    for i in range(len(clues)):
        draw_clues_on_canvas(categories, clues[i], dwg, Xc + odstep * width_clue,
                             840 - Yc + width_clue * (i + additional_rows), i, width_clue)
        if clues[i]["typ"] == 4:
            additional_rows += 1
