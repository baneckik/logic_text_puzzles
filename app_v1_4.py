...
window.title("Text puzzle generator v1.4")
    ...
    if choice_int == 3:
        ...
        for j, cat in enumerate(puzzle1.categories):
            ...
            if cat["typ"] == 'categorical' or cat["typ"] == 'ordinal':
                l = Label(cat_frame, text="Category " + str(j) + code + ":",
                ...
                l = Label(num_frame, text="Category " + str(j) + code + ":",
                          font=(std_font, std_font_size // 2, 'bold'))
                ...
                multiplicatives = [clue["oper"] == "*" for clue in puzzle1.clues if
                                   clue["typ"] == 3 and clue["K6"] == j]
                ...
                if can_first_el_be_changed or can_increment_be_changed:
                    ...
                    ev_btn = Button(eval_frame, text="eval", font=("Arial", std_font_size // 2),
...
def clicked_gen():
    ...
    for i in clue_order:
        ...
        if puzzle1.is_grid_completed() and not puzzle1.is_grid_contradictory():
            ...
            print("Restricting clue " + str(trace_i) + "/" + str(trace_clue_count) + " finished, type: " + str(
        ...
        if cat["typ"] == 'categorical' or cat["typ"] == 'ordinal':
            ...
            for i in range(k):
                ...
                if "groups" in cat and len(unique(cat["groups"])) > 1:
                    g_label = Label(object_frame, text="gr." + str(cat["groups"][i]),
    ...
    final_name.configure(text="p" + str(puzzle1.seed) + "K" + str(puzzle1.K) + "k" + str(puzzle1.k) + "c" + str(
        len(puzzle1.clues)) + ".pdf")
    final_name2.configure(text="p" + str(puzzle1.seed) + "K" + str(puzzle1.K) + "k" + str(puzzle1.k) + "c" + str(
        len(puzzle1.clues)) + ".svg")
    final_name3.configure(text="sol" + str(puzzle1.seed) + "K" + str(puzzle1.K) + "k" + str(puzzle1.k) + "c" + str(
        len(puzzle1.clues)) + ".pdf")
...
def clicked_sol():
    ...
    final_name3.configure(
        text="sol" + str(final_puzzle.seed) + "K" + str(final_puzzle.K) + "k" + str(final_puzzle.k) + "c" + str(
            len(final_puzzle.clues)) + ".pdf")
...
def clicked_eval(j):
    ...
    if digits_r > 3:
        messagebox.showwarning('Warning',
                               'Maximum number of digits after the decimal point in the increment/multiplier is 3!')
        return
    if final_puzzle.categories[j]["typ"] == "numerical" and \
            final_puzzle.categories[j]["seq_scheme"] == "geometric" and digits_r > 2:
        messagebox.showwarning('Warning', 'Maximum number of digits after the decimal point in the multiplier is 2!')
        return
    if final_puzzle.categories[j]["typ"] == "numerical" and \
            final_puzzle.categories[j]["seq_scheme"] == "geometric" and digits_a + digits_r * (final_puzzle.k - 1) > 9:
        messagebox.showwarning('Warning',
                               'Numbers generated could potentially have more than maximum number of digits after '
                               'the decimal point which is 10! Please specify numbers with less digits after comma.')
        return

    if final_puzzle.categories[j]["typ"] == "numerical" and \
            final_puzzle.categories[j]["seq_scheme"] == "geometric" and float(num_cats[n][8].get()) <= 0:
        messagebox.showwarning('Warning', 'First element in geometric sequence has to be greater than zero!')
        return
    if final_puzzle.categories[j]["typ"] == "numerical" and \
            final_puzzle.categories[j]["seq_scheme"] in ["arithmetic", "ascending"] and \
            float(num_cats[n][10].get()) <= 0:
        messagebox.showwarning('Warning', 'Sequence increment has to be greater than zero!')
        return
    if final_puzzle.categories[j]["typ"] == "numerical" and \
            final_puzzle.categories[j]["seq_scheme"] == "geometric" and float(num_cats[n][10].get()) <= 1:
...
choice = IntVar()
rad1 = Radiobutton(choose_frame, text='Generate random puzzle', value=1, variable=choice, command=chosen_random,
                   font=font)
rad2 = Radiobutton(choose_frame, text='Generate from random categories and seed: ', value=2, variable=choice,
                   command=chosen_seed, font=font)
rad3 = Radiobutton(choose_frame, text='Generate from custom categories and seed: ', value=3, variable=choice,
                   command=chosen_custom, font=font)
...
solution_btn = Button(final_frame, text="Get step-by-step solution!", font=("Arial", std_font_size),
                      command=clicked_sol)
