...
def draw_clues_on_canvas(categories, clue, c, X, Y, no, width):
    ...
    text_width = stringWidth(text0, normal_font, width)
    x += text_width + 1
    ...
    if clue["typ"] == 1:
        ...
        text_width = stringWidth(text1, special_font, width)
        x += text_width + 1
        ...
        if "i3" not in clue:
            ...
            text_width = stringWidth(text2, normal_font, width)
            x += text_width + 1
            ...
            c.setFont(special_font, width)
            c.drawString(x, Y, text3)
            text_width = stringWidth(text3, special_font, width)
            x += text_width + 1

            text4 = " ani do "
            c.setFont(normal_font, width)
            c.drawString(x, Y, text4)
            text_width = stringWidth(text4, normal_font, width)
            x += text_width + 1

            text5 = funs.get_string_name(categories, clue["K2"], clue["i3"], replace_polish, add_info=add_info)
            c.setFont(special_font, width)
            c.drawString(x, Y, text5)
            text_width = stringWidth(text5, special_font, width)
            x += text_width + 1
            if "i4" in clue:
                text6 = " ani do "
                c.setFont(normal_font, width)
                c.drawString(x, Y, text6)
                text_width = stringWidth(text6, normal_font, width)
                x += text_width + 1
        ...
        text1 = "Pod względem "
        c.drawString(x, Y, text1)
        text_width = stringWidth(text1, normal_font, width)
        x += text_width + 1

        text2 = "Kategorii " + str(clue["K6"])
        c.setFont(special_font, width)
        c.drawString(x, Y, text2)
        text_width = stringWidth(text2, special_font, width)
        x += text_width + 1

        text3 = " zachodzi: "
        c.setFont(normal_font, width)
        c.drawString(x, Y, text3)
        text_width = stringWidth(text3, normal_font, width)
        x += text_width + 1
        ...
        c.setFont(special_font, width)
        c.drawString(x, Y, text4)
        text_width = stringWidth(text4, special_font, width)
        x += text_width + 1

        text5 = "<"
        c.setFont(normal_font, width)
        c.drawString(x, Y, text5)
        text_width = stringWidth(text5, normal_font, width)
        x += text_width + 1
        ...
        c.setFont(special_font, width)
        c.drawString(x, Y, text6)
        text_width = stringWidth(text6, special_font, width)
        x += text_width + 1
        ...
        c.setFont(normal_font, width)
        c.drawString(x, Y, text7)
        text_width = stringWidth(text7, normal_font, width)
        x += text_width + 1
        ...
        text_width = stringWidth(text10, special_font, width)
        x += text_width + 1

        text11 = " pasuje do "
        c.setFont(normal_font, width)
        c.drawString(x, Y - width, text11)
        text_width = stringWidth(text11, normal_font, width)
        x += text_width + 1
        ...
        if clue["exclusive"]:
            ...
            if "g4" in clue:
                text8 = "obiekt z Kat." + str(clue["K4"]) + "gr." + str(clue["g4"])
            else:
                text8 = funs.get_string_name(categories, clue["K4"], clue["i4"], replace_polish, add_info=add_info)
            c.setFont(special_font, width)
            c.drawString(x, Y, text8)
            text_width = stringWidth(text8, special_font, width)
            x += text_width + 1
...
def draw_grid(puzzle1, c, X=30, Y=30, puzzle_h=400):
    ...
    K_cat = puzzle1.K  # number of categories
    k_cat = puzzle1.k  # number of objects in each category
    ...
    for row in range(N_rows):
        ...
        c.rect(X + text_box_size + row * box_size, Y + row * box_size, box_size,
               text_box_size + (N_rows - row) * box_size)
    c.setLineWidth(1)
    for row in range(N_rows):
        for k in range(k_cat):
            c.rect(X, Y + row * box_size, text_box_size + (row + 1) * box_size, k * box_size / k_cat)
            c.rect(X + text_box_size + row * box_size, Y + row * box_size, k * box_size / k_cat,
                   text_box_size + (N_rows - row) * box_size)
    ...
    for i, category in enumerate(categories):
        ...
        if category['cross_bar'] != "":
            ...
            if i != 0:
                c.rect(X + text_box_size + box_size * (i - 1),
                       Y + (K_cat - 1) * box_size + text_box_size - box_size / k_cat, box_size, box_size / k_cat,
                       fill=1)
            c.restoreState()
            ...
            if i != 0:
                draw_into_rectangle(c, X + text_box_size + box_size * (i - 1) + odstep * width,
                                    Y + (K_cat - 1) * box_size + text_box_size - box_size / k_cat + odstep * width,
                                    text.upper(), "default_font", width, box_size)
            # horizontal bar text on the left
            if i != 1:
                c.saveState()
                c.rotate(90)
                if i != 0:
                    draw_into_rectangle(c, Y + (i - 2) * box_size + odstep * width,
                                        -X + odstep * width - box_size / k_cat, text.upper(), "default_font", width,
                                        box_size)
                else:
                    draw_into_rectangle(c, Y + (K_cat - 2) * box_size + odstep * width,
                                        -X + odstep * width - box_size / k_cat, text.upper(), "default_font", width,
                                        box_size)
                c.restoreState()

        for j, name in enumerate(nazwy):
            if category['cross_bar'] != "":
            ...
            # horizontal texts:
            if miejsce == 1:
                draw_into_rectangle(c, X + odstep * width + odstep2,
                                    Y + (K_cat - 1) * box_size - (j + 1) * width + odstep * width, name.upper(),
                                    "default_font", width, space_size)
            elif miejsce != 2:
                draw_into_rectangle(c, X + odstep * width + odstep2,
                                    Y + (miejsce - 2) * box_size - (j + 1) * width + odstep * width, name.upper(),
                                    "default_font", width, space_size)
            c.saveState()
            # vertical texts:
            c.rotate(90)
            if miejsce == 2:
                draw_into_rectangle(c, Y + (K_cat - 1) * box_size + odstep * width,
                                    -X - text_box_size - (j + 1) * width + odstep * width, name.upper(), "default_font",
                                    width, space_size)
            elif miejsce != 1:
                draw_into_rectangle(c, Y + (K_cat - 1) * box_size + odstep * width,
                                    -X - text_box_size - (miejsce - 2) * box_size - (j + 1) * width + odstep * width,
                                    name.upper(), "default_font", width, space_size)
...
def draw_on_canvas(puzzle1, c, X=30, Y=30, puzzle_h=400):
    ...
    for i in range(len(clues)):
        draw_clues_on_canvas(categories, clues[i], c, Xc + odstep * width_clue, Yc - width_clue * (i + additional_rows),
                             i, width_clue)
        if clues[i]["typ"] == 4:
            additional_rows += 1
    ...
    c.drawString(X + text_box_size + box_size + 10, Y + 2 * width_foot,
                 "Contradictory: " + str(puzzle1.is_grid_contradictory()))
    ...
    for step in puzzle1.solution.split(";")[:-1]:
        ...
        if not (step2[-1] in ["conc1", "conc2"] and prev_val in ["conc1", "conc2"]) and step2[-1] != prev_val:
            ...
            for step3 in signs_list:
                draw_sign(puzzle1, c, step3[0], int(step3[1]), int(step3[2]), int(step3[3]), int(step3[4]), X=X, Y=Y,
                          red=False)
